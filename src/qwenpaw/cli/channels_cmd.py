...
# ── XMPP 配置 ───────────────────────────────────────────────────
def configure_xmpp(current_config: XMPPConfig) -> XMPPConfig:
    click.echo("\n=== Configure XMPP Channel ===")
    ...
    jid = click.prompt(
        "XMPP JID (e.g., user@jabber.at)",
        default=current_config.jid or "",
        type=str,
    )
    current_config.jid = jid

    password = click.prompt(
        "XMPP Password",
        default=current_config.password or "",
        hide_input=True,
        type=str,
    )
    current_config.password = password

    nickname = click.prompt(
        "Nickname",
        default=current_config.nickname or "QwenPawBot",
        type=str,
    )
    current_config.nickname = nickname

    room = click.prompt(
        "MUC Room (optional, e.g., test@conference.jabber.at)",
        default=current_config.room or "",
        type=str,
    )
    current_config.room = room
...
def configure_discord(current_config: DiscordConfig) -> DiscordConfig:
    ...
    enabled = prompt_confirm("Enable Discord channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
    bot_prefix = click.prompt("Bot prefix (e.g., @bot)", default=current_config.bot_prefix or "", type=str)
    current_config.bot_prefix = bot_prefix
    bot_token = click.prompt("Discord Bot Token", default=current_config.bot_token or "", hide_input=True, type=str)
    current_config.bot_token = bot_token
    use_proxy = prompt_confirm("Use HTTP proxy?", default=bool(current_config.http_proxy))
    if use_proxy:
        http_proxy = click.prompt("HTTP proxy address (e.g., http://127.0.0.1:7890)", default=current_config.http_proxy or "", type=str)
        current_config.http_proxy = http_proxy
        use_proxy_auth = prompt_confirm("Does proxy require authentication?", default=bool(current_config.http_proxy_auth))
        if use_proxy_auth:
            http_proxy_auth = click.prompt("Proxy authentication (format: username:password)", default=current_config.http_proxy_auth or "", hide_input=True, type=str)
...
def configure_dingtalk(current_config: DingTalkConfig) -> DingTalkConfig:
    ...
    enabled = prompt_confirm("Enable DingTalk channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
    bot_prefix = click.prompt("Bot prefix (e.g., @bot)", default=current_config.bot_prefix or "", type=str)
    current_config.bot_prefix = bot_prefix
    client_id = click.prompt("DingTalk Client ID", default=current_config.client_id or "", type=str)
    current_config.client_id = client_id
    client_secret = click.prompt("DingTalk Client Secret", default=current_config.client_secret or "", hide_input=True, type=str)
...
def configure_feishu(current_config: FeishuConfig) -> FeishuConfig:
    ...
    enabled = prompt_confirm("Enable Feishu channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
    ...
    domain = click.prompt("Region (feishu for China, lark for International)", default=current_domain, type=click.Choice(domain_choices))
    current_config.domain = domain
    bot_prefix = click.prompt("Bot prefix (e.g., @bot)", default=current_config.bot_prefix or "", type=str)
    current_config.bot_prefix = bot_prefix
    app_id = click.prompt("Feishu App ID", default=current_config.app_id or "", type=str)
    current_config.app_id = app_id
    app_secret = click.prompt("Feishu App Secret", default=current_config.app_secret or "", hide_input=True, type=str)
...
def configure_qq(current_config: QQConfig) -> QQConfig:
    ...
    enabled = prompt_confirm("Enable QQ channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
    bot_prefix = click.prompt("Bot prefix (e.g., @bot)", default=current_config.bot_prefix or "", type=str)
    current_config.bot_prefix = bot_prefix
    app_id = click.prompt("QQ App ID", default=current_config.app_id or "", type=str)
    current_config.app_id = app_id
    client_secret = click.prompt("QQ Client Secret", default=current_config.client_secret or "", hide_input=True, type=str)
    current_config.client_secret = client_secret
    markdown_enabled = prompt_confirm("Enable QQ markdown replies?", default=current_config.markdown_enabled)
...
def configure_telegram(current_config: TelegramConfig) -> TelegramConfig:
    ...
    enabled = prompt_confirm("Enable Telegram channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
    bot_prefix = click.prompt("Bot prefix (e.g., @bot)", default=current_config.bot_prefix or "", type=str)
    current_config.bot_prefix = bot_prefix
    bot_token = click.prompt("Telegram Bot Token", default=current_config.bot_token or "", hide_input=True, type=str)
    ...
    if not token:
        ...
        current_config.enabled = False
        return current_config
    show_typing = prompt_confirm("Show typing indicator?", default=current_config.show_typing is not False)
...
def configure_voice(current_config: VoiceChannelConfig) -> VoiceChannelConfig:
    click.echo("\n=== Configure Twilio Channel ===")
    enabled = prompt_confirm("Enable Twilio channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
    twilio_account_sid = click.prompt("Twilio Account SID", default=current_config.twilio_account_sid or "", type=str)
    current_config.twilio_account_sid = twilio_account_sid
    twilio_auth_token = click.prompt("Twilio Auth Token", default=current_config.twilio_auth_token or "", hide_input=True, type=str)
    current_config.twilio_auth_token = twilio_auth_token
    phone_number = click.prompt("Phone number (e.g., +15551234567, blank to provision later)", default=current_config.phone_number or "", type=str)
    current_config.phone_number = phone_number
    phone_number_sid = click.prompt("Phone Number SID (e.g., PN..., blank to provision later)", default=current_config.phone_number_sid or "", type=str)
    current_config.phone_number_sid = phone_number_sid
    configure_tts = prompt_confirm("Configure TTS/STT settings? (default: Google TTS + Deepgram STT)", default=False)
    if configure_tts:
        tts_provider = click.prompt("TTS provider", default=current_config.tts_provider or "google", type=str)
        current_config.tts_provider = tts_provider
        tts_voice = click.prompt("TTS voice", default=current_config.tts_voice or "en-US-Journey-D", type=str)
        current_config.tts_voice = tts_voice
        stt_provider = click.prompt("STT provider", default=current_config.stt_provider or "deepgram", type=str)
        current_config.stt_provider = stt_provider
        language = click.prompt("Language", default=current_config.language or "en-US", type=str)
        current_config.language = language
    welcome_greeting = click.prompt("Welcome greeting", default=current_config.welcome_greeting or "Hi! This is QwenPaw. How can I help you?", type=str)
...
def configure_console(current_config: ConsoleConfig) -> ConsoleConfig:
    ...
    enabled = prompt_confirm("Enable Console channel?", default=current_config.enabled)
    if not enabled:
        current_config.enabled = False
        return current_config
    current_config.enabled = True
...
_ALL_CHANNEL_CONFIGURATORS = {
    ...
    "xmpp": ("XMPP", configure_xmpp),
}


def _plugin_configure(_key: str, configurator, current):
...
def get_channel_configurators() -> dict:
    ...
    def _default_plugin_configure(current):
        ...
        _set(current, "enabled", prompt_confirm("Enable this channel?", default=enabled))
        prefix = _get(current, "bot_prefix", "") or ""
        _set(current, "bot_prefix", click.prompt("Bot prefix (e.g. [BOT])", default=prefix, type=str))
        return current
    ...
    for key in available:
        ...
        if ch_cls is None:
            continue
        display = getattr(ch_cls, "display_name", None) or key.replace("_", " ").title()
...
def configure_channels_interactive(config: Config) -> None:
    ...
    while True:
        ...
        for channel_key, (channel_name, _) in configurators.items():
            ...
            channel_choices.append((f"{channel_name} [{status}]", channel_key))
        ...
        if current_config is None:
            ...
            default = getattr(ch_cls, "get_default_config", lambda: None)() if ch_cls else None
    ...
    enabled_channels = [name for key, (name, _) in configurators.items() if _channel_enabled(_get_channel_config(config, key))]
    if enabled_channels:
...
@click.option("--agent-id", default="default", help="Agent ID (defaults to 'default')")
def list_cmd(agent_id: str) -> None:
    try:
        ...
        for key, name in _get_channel_names().items():
            ...
            if ch is None:
                continue
            status = click.style("enabled", fg="green") if _channel_enabled(ch) else click.style("disabled", fg="red")
            click.echo(f"\n{'─' * 40}")
...
def _install_channel_to_dir(key: str, from_path: str | None = None, from_url: str | None = None) -> None:
    CUSTOM_CHANNELS_DIR.mkdir(parents=True, exist_ok=True)
    if not key.isidentifier():
        click.echo(f"Key must be a valid Python identifier (e.g. my_channel), got: {key}", err=True)
        raise SystemExit(1)
    ...
    if dest_file.exists() or dest_dir.exists():
        click.echo(f"Channel '{key}' already exists in {CUSTOM_CHANNELS_DIR}. Edit the file or use --path/--url to overwrite.", err=True)
        raise SystemExit(1)
    dest_file.write_text(CHANNEL_TEMPLATE.format(key=key), encoding="utf-8")
    click.echo(f"✓ Created {dest_file}. Edit and add config with `qwenpaw channels config`.")

@channels_group.command("install")
@click.argument("key", required=True)
@click.option("--path", "from_path", type=click.Path(exists=True), help="Copy channel from local path (file or dir).")
@click.option("--url", "from_url", type=str, help="Download channel module from URL (.py file).")
...
@click.option("--path", "from_path", type=click.Path(exists=True), help="Copy channel from local path (file or dir).")
@click.option("--url", "from_url", type=str, help="Download channel module from URL (.py file).")
@click.option("--configure/--no-configure", default=True, help="Run interactive configurator after adding to config.")
def add_cmd(key: str, from_path: str | None, from_url: str | None, configure: bool) -> None:
    dest_file = CUSTOM_CHANNELS_DIR / f"{key}.py"
    dest_dir = CUSTOM_CHANNELS_DIR / key
...
@click.argument("key", required=True)
@click.option("--keep-config/--no-keep-config", "keep_config", default=False, help="Keep the channel entry in config.json (only remove the module).")
def remove_cmd(key: str, keep_config: bool) -> None:
    if key in BUILTIN_CHANNEL_KEYS:
        click.echo(f"'{key}' is a built-in channel and cannot be removed. Disable it in config instead.", err=True)
        raise SystemExit(1)
    dest_file = CUSTOM_CHANNELS_DIR / f"{key}.py"
    dest_dir = CUSTOM_CHANNELS_DIR / key
    if not dest_file.exists() and not dest_dir.exists():
        click.echo(f"Channel '{key}' not found in {CUSTOM_CHANNELS_DIR}.", err=True)
        raise SystemExit(1)
    import shutil
    if dest_file.exists():
        dest_file.unlink()
...
def configure_cmd(agent_id: str) -> None:
    try:
        ...
        temp_config = Config()
        temp_config.channels = agent_config.channels or temp_config.channels
...
@click.option("--agent-id", required=True, help="Agent ID sending the message")
@click.option("--channel", required=True, help="Target channel (e.g., console, dingtalk, feishu, discord, imessage, qq, xmpp)")
@click.option("--target-user", required=True, help="Target user ID")
@click.option("--target-session", required=True, help="Target session ID")
@click.option("--text", required=True, help="Text message to send")
@click.option("--base-url", default=None, help="Override the API base URL.")
@click.pass_context
def send_cmd(ctx: click.Context, agent_id: str, channel: str, target_user: str, target_session: str, text: str, base_url: Optional[str]) -> None:
